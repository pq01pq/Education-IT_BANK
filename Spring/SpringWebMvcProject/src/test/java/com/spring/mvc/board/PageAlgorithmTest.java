...
public class PageAlgorithmTest {
	...
	*** 페이징 알고리즘 만들기 ***
	...
	- 만약에 총 게시물의 수가 67개라면
	1 2 3 4 5 6 7 // 1 ~ 6 : 10개, 7 : 7개
	
	- 총 게시물의 수가 142개, 현재 사용자가 12페이지에 머물러있다면
	[이전] 11 12 13 14 15
	...
	- 만약 현재 사용자가 보고있는 페이지가 3페이지이고,
	한 화면에 보여줄 페이지 버튼이 10개씩이라면
	-> 끝 페이지 번호 : 10번
	- 만약 현재 페이지가 36페이지이고,
	한 화면에 보여줄 페이지 버튼이 20개라면
	-> 끝 페이지 번호 : 40번
	...
	5. 끝 페이지 보정
	- 총 게시물의 수가 323개이고, 한 페이지당 10개의 게시물을 보여줌
	- 이 사람은 31페이지를 보고있음
	- 한 화면에 버튼은 10개가 배치됨
	- 그렇다면 위 공식에 의한 끝 페이지는 몇번으로 계산? -> 40번
	- 하지만 실제 끝페이지는 몇번에서 끝나나 -> 33번
	
	5-1. 이전 버튼 활성 여부 설정
	- 언제 이전 버튼을 활성/비활성화 시킬 것인가의 여부
	-> 시작페이지가 1인 부분에서 비활성, 나머지는 다 활성
	...
	5-3. 끝 페이지 값 보정
	- 다음 버튼이 비활성화 되었을 때, 총 게시물 수에 맞춰 끝 페이지 번호를 재 보정
	...
	*/
	...
	public void pagingAlgorithmTest() {
		...
		paging.setPage(31);
		int displayPage = 10; // 페이지 버튼 갯수
		
		int endPage = (int)Math.ceil((double)paging.getPage() / displayPage) * displayPage;
