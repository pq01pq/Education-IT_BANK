...
<script>
	...
	$(function(){
		...
		// 입력값중 하나라도 만족하지 못한다면 회원가입 처리를 막기 위한 논리형 변수 선언
		let chk1 = false, chk2 = false, chk3 = false, chk4 = false;
		
		// 회원 가입시 사용자의 입력값 검증
		// id 입력값 검증
		$("#user_id").on('keyup', function() {	// 키를 눌렀다 뗐을 때
			if($("#user_id").val() === "") {	// 값이 없을 때 경고
				$("#user_id").css("background-color", "pink");
				$("#idChk").html('<b style="font-size:14px; color:red;">[아이디는 필수정보입니다]</b>');
				chk1 = false;
			} else if(!getIdCheck.test($("#user_id").val())) {	// 아이디 입력값 유효성 검사
				$("#user_id").css("background-color", "pink");
				$("#idChk").html('<b style="font-size:14px; color:red;">[영문자, 숫자 4-14자로 작성]</b>');
				chk1 = false;
			} else {	// 아이디 중복 확인 비동기 처리
				// 아이디 중복 확인 통신을 위해 아이디값 가져오기
				const id = $(this).val();
				console.log(id);
				...
				$.ajax({
					...
					success : function(result) {
						...
						if(result === "ok"){
							...
							$("#idChk").html('<b style="font-size:14px; color:green;">[사용 가능한 아이디]</b>');
							chk1 = true;
						} else {
							$("#user_id").css("background-color", "pink");
							$("#idChk").html('<b style="font-size:14px; color:red;">[중복된 아이디]</b>');
							chk1 = false;
						}
					},
					error : function() {
		...
		$("#password_check").on('keyup', function() {
			...
			if($(this).val === ""){
				...
				$("#pwChk2").html('<b style="font-size:14px; color:red;">[비밀번호 확인은 필수정보]</b>');
				chk3 = false;
			}
			// 비밀번호 확인란 유효성 검사
			else if($("#password").val() != $(this).val()){
				$("#password_check").css("background-color", "pink");
				$("#pwChk2").html('<b style="font-size:14px; color:red;">[비밀번호가 일치하지 않음]</b>');
				chk3 = false;
			}
			else {
				$("#password_check").css("background-color", "aqua");
				$("#pwChk2").html('<b style="font-size:14px; color:green;">[비밀번호 확인 끝]</b>');
				chk3 = true;
			}
		}); // 비밀번호 확인 검증 끝
		...
		$("#signup-btn").click(function() {
			if(chk1 && chk2 && chk3 && chk4) {
				...
				// 비동기 통신 시작
				// 클라이언트에서 서버와 통신하는 ajax 함수
				$.ajax({
					type : "POST",	// 서버에 전송하는 http 방식
					url : "/user/",	// 서버에 요청하는 url
					headers : {
						"Content-Type": "application/json"
					},	// 요청 헤더 정보
					dataType : "text",	// 서버로부터 응답받을 데이터의 형태
					data : JSON.stringify(user),	// 서버로 전송할 데이터(한개만 보내야함. json 형태로 변환)
					success : function(result) {
					// 서버와의 통신 성공시 실행할 내용. 매개변수는 통신 성공시 데이터가 저장될 곳
						console.log("통신 성공 : " + result);
		...
		$("#signInId").on("keyup", function() {
			if($(this).val() === ""){
				...
				$("#idCheck").html('<b style="font-size:14px; color:red;">[아이디는 필수정보]</b>');
				chk1 = false;
			}
			// 아이디 유효성 검사
		...
		// 비밀번호 입력창에 엔터키 입력 시 로그인 버튼 클릭 이벤트 실행
				...
				$.ajax({
					...
					success : function(result) {
						...
						if(result === "loginSuccess") {
							...
							$('#signInId').css("background-color", "pink");
							$('#idCheck').html('<b style="font-size:14px;color:red;">[존재하지 않는 아이디]</b>');
							$("#signInPw").val("");
							$("#signInId").focus(); // 커서를 이동시키고, 스크롤도 해당 위치로 이동시키는 메서드
							chk1 = false;
						} else if(result === "pwFail"){
							$('#signInPw').css("background-color", "pink");
							$('#pwCheck').html('<b style="font-size:14px;color:red;">[비밀번호 다시 확인]</b>');
							$("#signInPw").val("");
							$("#signInPw").focus();
							chk2 = false;
						}
